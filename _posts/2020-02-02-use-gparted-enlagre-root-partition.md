...
* 因为本文一开始是摸索着来解决扩容问题，走了一个弯路，即通过XManager来展示gparted界面，尝试扩展分区，但是失败了，此路不通。本文记录了这个操作，证实在正在运行的系统里直接运行gparted是没法扩展分区的，正确的开始步骤，请从“借助gparted live ISO进行修改”开始(正确的步骤里，用不到XManager工具，即不需要安装它;走弯路的步骤才需要安装它)！！
