...
Locust是一个比较容易上手的、开源的分布式用户负载测试工具，它允许你使用Python代码来定义用户行为，并模拟数以万计的用户对网站或应用程序进行压力测试。Locust 在英文中是 蝗虫 的意思：作者的想法是在测试期间，放一大群 蝗虫 攻击您的网站。当然事先是可以用 Locust 定义每个蝗虫（或测试用户）的行为，并且通过 Web UI 实时监视围攻过程。由于其灵活性及易用性，Locust在开发者和测试工程师中相当受欢迎。
